const Database = {
  personalInfo: {
    fullName: 'Suka Sukaesa',
    email: 'sukasukaesa@gmail.com',
    phoneNumber: '0857 2772 1799',
    address: 'Surakarta, Jawa Tengah',
  },

  sections: {
    educations: [
      {
        degree: 'Magister Informatika',
        schoolName: 'Universtias Gadjah Mada',
        location: 'Surakarta, Jawa Tengah',
        startDate: 'September 2020',
        endDate: 'September 2024',
      },
      {
        degree: 'Sarjana Informatika',
        schoolName: 'Universtias Sebelas Maret',
        location: 'Surakarta, Jawa Tengah',
        startDate: 'September 2020',
        endDate: 'September 2024',
      },
    ],

    experiences: [
      {
        companyName: 'Digital Tech',
        positionTitle: 'Manajer of FrontEnd',
        location: 'New York City, US',
        description: 'Make all front end oke',
        startDate: 'September 2020',
        endDate: 'present',
      },
      {
        companyName: 'Indo Digital',
        positionTitle: 'FrontEnd Developer',
        location: 'Jakarta, Indonesia',
        description: 'Building a website',
        startDate: 'Agustus 2019',
        endDate: 'Agustus 2020',
      },
    ],
  },
};

export default Database;
